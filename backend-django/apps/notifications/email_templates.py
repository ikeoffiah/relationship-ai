...
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  <h2 style="color:#2D3748;">Welcome to Bliss</h2>
  ...
  <p style="color:#A0AEC0;font-size:12px;">
    <strong>Bliss is an AI-powered support tool, not a licensed therapy service.</strong>
...
def email_partner_invite(accept_url: str, inviter_name: str = "Someone you know") -> tuple[str, str]:
    """2. Partner Invite"""
    subject = f"{inviter_name} invited you to Bliss"
    html = f"""
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  <h2 style="color:#2D3748;">You've been invited to Bliss</h2>
  <p style="color:#4A5568;">{inviter_name} has invited you to try Bliss together.</p>
  ...
  <hr style="border:none;border-top:1px solid #E2E8F0;margin:24px 0;">
  <p style="color:#A0AEC0;font-size:12px;">
    Bliss is an AI-powered support tool, not a licensed therapy service.
  </p>
</div>"""
    return subject, html
...
def email_password_reset(reset_url: str) -> tuple[str, str]:
    """3. Password Reset"""
    subject = "Reset your Bliss password"
    html = f"""
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
...
def email_erasure_complete() -> tuple[str, str]:
    ...
    subject = "Your data has been deleted — Bliss"
    html = """
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  <h2 style="color:#2D3748;">Your data has been deleted</h2>
  <p style="color:#4A5568;">All your personal data has been permanently removed from Bliss,
...
def email_gdpr_export_ready(download_url: str) -> tuple[str, str]:
    ...
    subject = "Your data export is ready — Bliss"
    html = f"""
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
...
def email_relay_waiting(partner_name: str) -> tuple[str, str]:
    ...
    subject = f"{partner_name} sent you a reflection — Bliss"
    html = f"""
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  <h2 style="color:#2D3748;">A message is waiting for you</h2>
  <p style="color:#4A5568;">{partner_name} has sent you a reflection via Bliss's async relay.</p>
...
def email_therapist_connected(therapist_name: str) -> tuple[str, str]:
    ...
    subject = "A therapist has connected to your account — Bliss"
    html = f"""
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  <h2 style="color:#2D3748;">A therapist has connected</h2>
  <p style="color:#4A5568;"><strong>{therapist_name}</strong> has been connected to your Bliss account
  with your consent.</p>
...
<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;">
  ...
  <p style="color:#718096;font-size:13px;">
    Bliss is not a crisis service. Please use the resources above if you need immediate help.
  </p>
</div>"""
    return subject, html
